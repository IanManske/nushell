...
impl Command for Input {
    ...
    fn run(
        ...
        match input(prompt, bytes_until, suppress_output, numchar.item) {
            Ok(Some(buf)) => Ok(Value::string(buf, call.head).into_pipeline_data()),
            Ok(None) => Err(ShellError::InterruptedByUser {
                span: Some(call.head),
            })?,
            Err(err) => Err(err.into_spanned(call.head))?,
        }
    }
...
fn input(
    ...
    loop {
        if i64::try_from(buf.len()).unwrap_or(0) >= numchar {
            break;
        }
